        ...
        try:
            for slam_map in maps:
                ...
                for i, cone_with_stats in enumerate(mapped_cones):
                    x = cone_with_stats["x"]
                    y = cone_with_stats["y"]
                    covariance = cone_with_stats["covariance"]
...
def read_map_csv_real_data(path, animate_maps = False):
    """
    ...
    with open(path) as f:
        ...
        for i, row in enumerate(rows):  # Every row contains data for a single cone
            ...
            if animate_maps and int(row[4]) == 0 and i != 0:
                map_frames.append([x_vals, y_vals, cov])
                x_vals, y_vals = [], []
        if animate_maps:
            return map_frames
        return x, y, times
...
def run_real_cone_data():
    ...
    map_frames = read_map_csv_real_data("csv_files/real_cone_data.csv", animate_maps=True)
    ani = MapAnimation(map_frames)
    ani.animate()
